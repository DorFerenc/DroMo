...
def test_preprocess_video_success(client, mongo):
    ...
    # First, upload a video
    upload_data = {
        'title': 'Preprocess Test Video',
        'file': (BytesIO(b'fake video content for preprocess'), 'preprocess_test.mp4')
    }
    upload_response = client.post('/api/upload', data=upload_data, content_type='multipart/form-data')
    upload_result = json.loads(upload_response.data.decode('utf-8'))
    video_id = upload_result['video_id']
...
def test_progress_process_video_success(client, mongo):
    ...
    upload_data = {
        ...
        'file': (BytesIO(b'fake video content for progress'), 'progress_test.mp4')
    }
    upload_response = client.post('/api/upload', data=upload_data, content_type='multipart/form-data')
    upload_result = json.loads(upload_response.data.decode('utf-8'))
    video_id = upload_result['video_id']
    ...
    assert 'frames_processed' in progress_data
    assert 'frames_directory' in progress_data
